...
#+end_src
*** Pyright
Pyright likes to scan way too many projects and results in too many open files errors on macOS. If using doom set ~lsp-pyright-multi-root~ to ~nil~ in =./modules/lang/python/config.el=.
#+begin_src sh :tangle no
